...
//
					if (*cause == SWITCH_CAUSE_NORMAL_CLEARING) {
						const char* loopback_b_hangup_cause = switch_channel_get_variable(peer_channel, "loopback_b_hangup_cause");
						if (!switch_strlen_zero(loopback_b_hangup_cause)) {
							*cause = switch_channel_str2cause(loopback_b_hangup_cause);
						}
					}
...
//
						if (*cause == SWITCH_CAUSE_NORMAL_CLEARING) {
							const char* loopback_b_hangup_cause = switch_channel_get_variable(originate_status[i].peer_channel, "loopback_b_hangup_cause");
							if (!switch_strlen_zero(loopback_b_hangup_cause)) {
								*cause = switch_channel_str2cause(loopback_b_hangup_cause);
							}
						}
...
//
				if (*cause != SWITCH_CAUSE_NORMAL_CLEARING) {
					const char *loopback_leg = switch_channel_get_variable(caller_channel, "loopback_leg");
					if (loopback_leg && *loopback_leg == 'B') {
						const char *other_loopback_leg_uuid = switch_channel_get_variable(caller_channel, "other_loopback_leg_uuid");
						if (!switch_strlen_zero(other_loopback_leg_uuid)) {
							...
							if (other_loopback_session) {
								...
								if (other_loopback_channel) {
									switch_channel_set_variable(other_loopback_channel, "loopback_b_hangup_cause", switch_channel_cause2str(*cause));
								}
